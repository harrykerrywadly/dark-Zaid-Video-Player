...
async def startvideo(client, m: Message):
    ...
    if not replied:
        ...
        try:
            ...
            await m.reply_photo(
                photo="https://telegra.ph/file/235db2efe992211306d87.png",
                reply_markup=keyboard,
                caption=f"💡 **video streaming started !**\n\n» **join to video chat on the top to watch the video.**")
            return await msg.delete()
        except Exception as e:
...
@Client.on_message(command(["vstop", f"end"]) & filters.group & ~filters.edited)
@authorized_users_only
...
@Client.on_message(command(["cplay", f"cplay@{Zaid.BOT_USERNAME}"]) & filters.group & ~filters.edited)
@authorized_users_only
...
@Client.on_message(command(["cstop", f"cend"]) & filters.group & ~filters.edited)
@authorized_users_only
